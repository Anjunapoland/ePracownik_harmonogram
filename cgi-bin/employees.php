...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($act==='toggle_role') {
        ...
        if ($id === $user['id']) { $msg='Nie możesz zmienić własnej roli'; $mt='danger'; }
            ...
            $row = $stmt->fetch();
            if ($row) {
                $newRole = $row['role']==='admin' ? 'employee' : 'admin';
                $db->prepare('UPDATE users SET role=? WHERE id=?')->execute([$newRole, $id]);
                $msg='Rola zmieniona na: '.($newRole==='admin'?'Administrator':'Pracownik'); $mt='success';
            }
        }
    }
    if ($act==='toggle_view') {
...
<div class="card">
<table class="dtable">
<thead><tr><th>Imię i nazwisko</th><th>Dział</th><th>Etat</th><th>Rola</th><?php if($globalViewMode==='individual'):?><th>Widok</th><?php endif;?><th>Konto</th><th>Hasło</th><th>E-mail</th><th>Akcje</th></tr></thead>
<tbody>
...
<tr>
    ...
    <td><span class="badge <?=$e['role']==='admin'?'badge-admin':'badge-emp'?>"><?=$e['role']==='admin'?'Admin':'Pracownik'?></span></td>
    ...
    <td>
        ...
        <form method="post" style="display:inline"><?=csrf_field()?><input type="hidden" name="action" value="toggle_view"><input type="hidden" name="uid" value="<?=$e['id']?>">
            <button class="badge-btn <?=$e['can_view_all']?'badge-view-on':'badge-view-off'?>"><?=$e['can_view_all']?'👁 Pełny widok':'🔒 Tylko swój'?></button>
        </form>
        <?php else: ?>
        <span class="badge badge-ok">Pełny (admin)</span>
        <?php endif; ?>
    </td>
    <?php endif; ?>
    <td><span class="badge <?=$e['active']?'badge-ok':'badge-danger'?>"><?=$e['active']?'Aktywne':'Zablokowane'?></span></td>
    <td><span class="badge <?=$e['must_change_password']?'badge-warn':'badge-ok'?>"><?=$e['must_change_password']?'Tymczasowe':'Ustawione'?></span></td>
    <td class="text-muted tsm"><?=h($e['email'])?></td>
    <td class="act-cell">
        ...
        <button class="btn btn-sm btn-ghost"><?=$e['active']?'Zablokuj':'Aktywuj'?></button></form>
        <?php if($e['id']!==$user['id']): ?>
        <form method="post" style="display:inline" onsubmit="return confirm('<?=$e['role']==='admin'?'Odebrać uprawnienia administratora?':'Nadać uprawnienia administratora?'?>')">
        <?=csrf_field()?><input type="hidden" name="action" value="toggle_role"><input type="hidden" name="uid" value="<?=$e['id']?>">
        <button class="btn btn-sm btn-ghost <?=$e['role']==='admin'?'btn-warn':''?>"><?=$e['role']==='admin'?'Odbierz admina':'Nadaj admina'?></button></form>
        <?php endif; ?>
        ...
        <button class="btn btn-sm btn-ghost">Reset hasła</button></form>
        <?php if($e['id']!==$user['id']): ?>
        <form method="post" style="display:inline" onsubmit="return confirm('UWAGA: Usunąć konto <?=h($e['full_name'])?>?\n\nTo trwale usunie użytkownika i wszystkie jego wpisy w harmonogramie.\n\nTej operacji nie można cofnąć!')">
        <?=csrf_field()?><input type="hidden" name="action" value="delete_user"><input type="hidden" name="uid" value="<?=$e['id']?>">
        <button class="btn btn-sm btn-danger">🗑 Usuń</button></form>
        <?php endif; ?>
    </td>
</tr>
<?php endforeach; ?>
