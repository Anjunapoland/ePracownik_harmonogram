...
function sendForm(){
    if(!document.getElementById('otName').value.trim()){alert('Wype\u0142nij imi\u0119 i nazwisko.');return;}
    var daysList=[];
    rows().forEach(function(r,i){
        var d=r.querySelector('.ot-date').value;
        var h=parseFloat(r.querySelector('.ot-hrs').value)||0;
        if(d) daysList.push(fmtD(d)+' - '+fmtN(h)+' godz.');
    });
    var data={
        'Imi\u0119 i nazwisko':document.getElementById('otName').value,
        'Stanowisko':document.getElementById('otPosition').value,
        'Data wniosku':fmtD(document.getElementById('otDate').value),
        'Nadgodziny na koncie':document.getElementById('otTotal').value+' godz.',
        'Dni do odbioru':daysList.join('; '),
        'Razem do odbioru':document.getElementById('otSumH').textContent,
